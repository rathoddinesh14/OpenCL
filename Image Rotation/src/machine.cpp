...
void Machine::create_single_context(int platform_id, int device_id) {
...
void Machine::create_build_program(const char *filename) {
    ...
    if (status != CL_SUCCESS) {
        ...
        clGetProgramBuildInfo(m_program, m_platforms[m_platform_id].devices[m_device_id], CL_PROGRAM_BUILD_LOG, sizeof(build_log), build_log, NULL);
        cout << build_log << endl;
        exit(1);
    }
}
...
void Machine::print_max_work_group_size() {
    size_t max_work_group_size;
    // get max work group size using clGetDeviceInfo
    clGetDeviceInfo(m_platforms[m_platform_id].devices[m_device_id], CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(size_t), &max_work_group_size, NULL);
    cout << "max work group size: " << max_work_group_size << endl;
}

void Machine::print_supported_image_formats() {
    cl_uint num_orders;
    cl_int status;
    status = clGetSupportedImageFormats(m_context, CL_MEM_READ_ONLY, CL_MEM_OBJECT_IMAGE2D, 0, NULL, &num_orders);
    check_point(status, "Failed to get number of supported image channel orders ");
    cl_image_format *orders = new cl_image_format[num_orders];
    status = clGetSupportedImageFormats(m_context, CL_MEM_READ_ONLY, CL_MEM_OBJECT_IMAGE2D, num_orders, orders, NULL);
    check_point(status, "Failed to get supported image channel orders ");

    for (int i = 0; i < num_orders; i++) {
        cout << "------------------------------------------------------------" << endl;
        cout << "Order " << i << ": " << endl;
        print_image_channel_type(orders[i].image_channel_data_type);
        print_image_order(orders[i].image_channel_order);
        cout << "------------------------------------------------------------" << endl;
    }
}
